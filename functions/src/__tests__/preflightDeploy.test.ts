 ...
 *
 * CRITICAL: WHAT THIS PINS AND WHY IT IS NOT A SOURCE-STRING ASSERTION. The gate's own
 ...
 *
 * KEY: THE CASE THAT EARNS THE FILE IS `behind`. A dirty-tree check is the obvious
 ...
 *
 * WARNING: The git-touching half is deliberately NOT tested. It shells out to
...
// CRITICAL: REQUIRED, NOT DECORATION. A test file with no import/export is a GLOBAL
...
describe('preflight:deploy — the decision table', () => {
  ...
  it('REFUSES a CLEAN tree that is behind origin/main — the stale-checkout case', () => {
    // KEY: The case a dirty-only gate cannot see. `dirty` is empty here on
  ...
  it('names the stamp path exactly, because the caller excludes it by equality', () => {
    // CRITICAL: If this path drifts from where gen-build-info.cjs writes, the exclusion
