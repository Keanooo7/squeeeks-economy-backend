...
//
// CRITICAL: THREE COPIES OF A BROKEN STRIPPER EXISTED BEFORE THIS FILE.
...
//
// WARNING: WHY BLOCK-COMMENTS-FIRST IS WRONG. A slash and an asterisk adjacent inside
...
//
// WARNING: WHY LINE-COMMENTS-FIRST IS ALSO WRONG. It moves the wound: the line-comment
// pass then truncates any string containing a double slash, so a `'https://…'`
// literal swallows the rest of its line.
//
// OK: No regex can do this, because whether a delimiter opens a comment depends
...
// while in code.
//
// NOTE: THIS IS CJS ON PURPOSE. `check-deployed.cjs` cannot import from `src/`: it
...
//
// NOTE: THE LIMIT, so it is not mistaken for a lexer: a regex literal containing an
